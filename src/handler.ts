...
export class MCPToolHandler {
    ...
    try {
      ...
      switch (call.name) {
        ...
        case "machina_vault_balance": {
          ...
          if (call.arguments.chain) params.set("chain", String(call.arguments.chain));
          if (call.arguments.token) params.set("token", String(call.arguments.token));
          const qs = params.toString();
          return this.apiGet(
            `/v1/vaults/${call.arguments.vault_id}/balances${qs ? `?${qs}` : ""}`,
          );
        }
        ...
        case "machina_vault_history": {
          ...
          if (call.arguments.chain) historyParams.set("chain", String(call.arguments.chain));
          if (call.arguments.limit) historyParams.set("limit", String(call.arguments.limit));
          const historyQs = historyParams.toString();
          return this.apiGet(
            `/v1/vaults/${call.arguments.vault_id}/history${historyQs ? `?${historyQs}` : ""}`,
          );
        }
  ...
  private async apiGet(path: string): Promise<MCPToolResult> {
  ...
  private async apiPost(path: string, body: Record<string, unknown>): Promise<MCPToolResult> {
    const res = await fetch(`${this.apiUrl}${path}`, {
      method: "POST",
  ...
  private async toResult(res: Response): Promise<MCPToolResult> {
